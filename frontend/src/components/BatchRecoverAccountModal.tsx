...
// API服务对象，封装与后端通信的方法
const infiniAccountApi = {
  // 发送验证码
  sendVerificationCode: async (email: string, type: number) => {
    try {
      const response = await api.post('/api/infini-accounts/verify', { email, type });
      return {
  ...
  // 获取验证码
  fetchVerificationCode: async (email: string) => {
    try {
      const response = await api.get(`/api/infini-accounts/verify-code?email=${email}`);
      return {
        success: response.data.success,
        message: response.data.message,
        data: response.data.data
  ...
  sendGoogle2faVerificationEmail: async (email: string, accountId: string) => {
    try {
      const response = await api.post('/api/infini-accounts/2fa/verify-email', { email, accountId });
      return {
        success: response.data.success,
        message: response.data.message,
        data: response.data.data
  ...
  // 绑定2FA
  bindGoogle2fa: async (accountId: string, verificationCode: string, google2faToken: string) => {
    try {
      const response = await api.post('/api/infini-accounts/2fa/bind', {
        accountId,
        verification_code: verificationCode,
        google_2fa_code: google2faToken
      });
      return {
        success: response.data.success,
        message: response.data.message,
...
const totpToolApi = {
  // 生成TOTP验证码
  generateTotpCode: async (accountId: string) => {
    try {
      const response = await api.get(`/api/totp-tools/generate/${accountId}`);
      return {
        success: response.data.success,
        message: response.data.message,
        data: response.data.data
